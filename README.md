...
![演讲者窗实拍：CURRENT/NEXT 双预览 + 逐字稿 + 计时器](docs/presenter-mode.png)

> 实拍：frontend-slides 生成的 deck 注入后按 `S` 弹出的**演讲者窗**——左上当前页、右上下一页（像素级预览，和观众看到的同一套渲染），下方逐字稿提词器 + 计时器，卡片可拖动缩放。观众窗（投影/外接屏）只显示当前页，两窗 `←` `→` 双向同步。
